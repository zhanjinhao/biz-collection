...
public class IdempotenceConfiguration implements ImportAware {
    ...
    public void setImportMetadata(AnnotationMetadata importMetadata) {
        this.annotationAttributes = AnnotationAttributes.fromMap(
            importMetadata.getAnnotationAttributes(EnableIdempotenceManagement.class.getName(), false));
        if (this.annotationAttributes == null) {
            throw new IllegalArgumentException(
                "@EnableIdempotenceManagement is not present on importing class " + importMetadata.getClassName());
        }
    }
