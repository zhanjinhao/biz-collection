package cn.addenda.bc.bc.tc.error;
        ...
        factory.addContextCustomizers((context) -> {
            ErrorReportValve valve = new JsonErrorReportValve() {
                ...
                protected void report(Request request, Response response, Throwable throwable) {
                    ...
                    if (statusCode == 404) {
                        ControllerResult<Object> controllerResult = new ControllerResult<>();
                        controllerResult.setReqStatus(Status.SYSTEM_EXCEPTION);
                        controllerResult.setReqFailedCode(-2);
                        controllerResult.setReqAttachment(request.getRequestURI() + ".");

                        doWrite(response, container, JacksonUtils.objectToString(controllerResult));
                    } else {
